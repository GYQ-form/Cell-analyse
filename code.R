...
##建立距离判别模型
distinguish <- analyse[,c("Age","gender","Cigarettes.per.Day")]
distinguish$smoke <- rep(NA,nrow(distinguish))
for(i in 1:nrow(distinguish)){
  if(is.na(analyse$Smoking.History_modified[i]))distinguish$smoke[i] <- NA
  else if(analyse$Smoking.History_modified[i]=="Non-Smoker")distinguish$smoke[i] <- 0
  else if(analyse$Smoking.History_modified[i]=="Reformed Smoker")distinguish$smoke[i] <- 1
  else if(analyse$Smoking.History_modified[i]=="Smoker")distinguish$smoke[i] <- 2
}
distinguish$type <- ifelse(analyse$`mutant level`=="high",2,1)
distinguish$Cigarettes.per.Day <- as.numeric(distinguish$Cigarettes.per.Day)
distinguish <- distinguish[complete.cases(distinguish),]
train <- distinguish[1:60,-5]
test <- distinguish[61:74,-5]

source('distinguish_distance.R', echo=TRUE)
res <- distinguish.distance(train,distinguish$type[1:60],TestX = test,var.equal = T)
t_distance <- table(res,distinguish$type[61:74])
ratio <- sum(res==distinguish$type[61:74])/length(res)
library(RColorBrewer)
mycolor <- brewer.pal(8,"Set2")

plot(t_distance,main="prediction of the remained 14 samples\n(Distance discrimination)",col=mycolor,xlab = "true class",
     ylab = "predict class",border="white")
title(sub = paste("right ratio:",round(ratio,2)),col.sub="red",line = 2)
location <- locator()
text(location$x,location$y,labels = c("4","5","5"),col = "purple",cex = 2)


#glm
distinguish$type <- distinguish$type-1
glm.fit <- glm(type~.,family = binomial,data = distinguish)
glm.pre <- predict(glm.fit,type = "response")
glm.pre <- ifelse(glm.pre>0.5,1,0)
table(glm.pre,distinguish$type)

#线性判别
library(MASS)
train <- distinguish[1:60,]
lda.fit <- lda(type~.,data = train)
lda.pre <- predict(lda.fit,test)
lda.pre2 <- predict(lda.fit)
t_line <- table(lda.pre$class,distinguish$type[61:74])
ratio_line <- sum(lda.pre$class==distinguish$type[61:74])/length(lda.pre$class)

mycolor <- brewer.pal(8,"Set2")

plot(t_line,main="prediction of the test group\n(Linear discrimination)",col=mycolor,xlab = "true class",
     ylab = "predict class",border="white")
title(sub = paste("right ratio:",round(ratio_line,2)),col.sub="red",line = 2)
location <- locator()
text(location$x,location$y,labels = c("4","10"),col = "purple",cex = 2)


t_line2 <- table(lda.pre2$class,distinguish$type[1:60])
ratio_line2 <- sum(lda.pre2$class==distinguish$type[1:60])/length(lda.pre2$class)

plot(t_line2,main="prediction of the training group\n(Linear discrimination)",col=mycolor,xlab = "true class",
     ylab = "predict class",border="white")
title(sub = paste("right ratio:",round(ratio_line2,2)),col.sub="red",line = 2)
location <- locator()
text(location$x,location$y,labels = c("4","12","2","42"),col = "purple",cex = 1.5)


#预测
my_test <- data.frame(Age=20,gender=1,Cigarettes.per.Day=0,smoke=1)
my_pre <- predict(lda.fit,my_test)
my_pre$class
